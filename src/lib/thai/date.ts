 ...
 *
 * ⚠️ **ช่องเลือกวันที่จะแสดงผลเป็นรูปแบบอะไรนั้นเราสั่งไม่ได้** — เบราว์เซอร์เรนเดอร์
 * ตาม locale ของตัวมันเอง ไม่ใช่ตาม `lang="th"` ของหน้า · เครื่องที่ตั้ง Chrome
 * เป็นอังกฤษจึงเห็น `mm/dd/yyyy` และต่อให้ตั้งเป็นไทยก็ยังเป็น ค.ศ. อยู่ดี
 *
 * ผู้ดูแลรับทราบและ**เลือกให้ปล่อยไว้แบบนี้** (27 ส.ค. 2569) — วันที่ที่ "แสดงเป็นข้อมูล"
 * ทุกจุดเป็น วว/ดด/ปปปป พ.ศ. ผ่าน `formatThaiDate()` แล้ว เหลือเฉพาะตัวช่องกรอกเท่านั้น
 * ทางเลือกที่เคยเสนอคือใส่ hint ใต้ช่อง หรือเขียน date picker แบบ พ.ศ. เอง —
 * **อย่าเพิ่งลงมือทำถ้าไม่ได้ถูกสั่งใหม่**
 */
