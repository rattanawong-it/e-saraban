...
describe("formatThaiDate", () => {
  ...
  it("ไม่ระบุรูปแบบ = วว/ดด/ปปปป ตามที่ผู้ดูแลกำหนด", () => {
    const d = new Date("2026-08-22T03:30:00Z")

    // ⚠️ ค่าปริยายคือ short ไม่ใช่ medium — จุดที่แสดงวันที่เป็น "ข้อมูล" ทั่วทั้งระบบ
    // เรียกโดยไม่ระบุรูปแบบ การเปลี่ยนค่าปริยายจึงเปลี่ยนหน้าจอทั้งระบบพร้อมกัน
    expect(formatThaiDate(d)).toBe("22/08/2569")
    expect(formatThaiDateTime(d)).toBe("22/08/2569 10:30")
  })
