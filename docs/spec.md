...
| **D18** | ไฟล์แนบใน P2 *(25 ส.ค. 2569)* | **ยังไม่เข้ารหัส** — เก็บเป็นไฟล์ธรรมดาผ่าน `StorageAdapter` (LocalFs) ไปก่อน · โครงสร้างฟิลด์เข้ารหัสของ `Attachment` (`isEncrypted` `encAlgo` `encryptedDek` `iv` `authTag` `keyVersion`) **มีอยู่ในฐานข้อมูลแล้วตั้งแต่ migration แรก** จึงเปิดใช้ทีหลังได้โดยไม่ต้อง migrate · การเข้ารหัส envelope ตาม §8.2 เลื่อนไป **P3** พร้อมนโยบายเอกสารลับ (§15 ข้อ 6 ที่ยังไม่ได้คำตอบ) |
...
> **สถานะการทำ (D18 · 25 ส.ค. 2569)** — P2 เก็บไฟล์แนบแบบ **ไม่เข้ารหัส** ไปก่อน
> ส่วนที่อยู่ใต้หัวข้อนี้ทั้งหมดเป็นขอบเขตของ **P3**
>
> | | P2 (ทำแล้ว) | P3 (ยังไม่ทำ) |
> |---|---|---|
> | ที่เก็บ | `LocalFsStorage` · ไฟล์ตั้งชื่อด้วย UUID เก็บนอก `public/` | เปลี่ยน adapter ได้โดยไม่แตะ business logic |
> | เนื้อไฟล์ | plaintext บนดิสก์ | envelope encryption ตามหัวข้อนี้ |
> | ฟิลด์ใน `Attachment` | `isEncrypted = false` · ฟิลด์กุญแจเป็น `null` | เติมค่าจริง ไม่ต้อง migrate |
> | การส่งไฟล์ | `/api/files/[id]` + ตรวจสิทธิ์ + audit | เพิ่มถอดรหัส stream + watermark |
>
> **เหตุผลที่แยกเฟส:** การเข้ารหัสผูกกับนโยบายเอกสารลับ (§15 ข้อ 6) ซึ่งยังไม่ได้คำตอบ
> ว่าองค์กรอนุญาตให้เอกสาร "ลับที่สุด" เข้าระบบอิเล็กทรอนิกส์หรือไม่ · ถ้าคำตอบคือไม่อนุญาต
> ขอบเขตของการเข้ารหัสจะเปลี่ยนไปทั้งหมด การทำล่วงหน้าจึงเสี่ยงเสียเปล่า
>
> ⚠️ **ข้อจำกัดที่ต้องรู้ระหว่างนี้:** ใครที่เข้าถึงดิสก์ของเซิร์ฟเวอร์ได้ จะอ่านไฟล์แนบได้ทั้งหมด
> โดยไม่ผ่าน `can()` และไม่ทิ้งร่องรอยใน audit log — **ห้ามนำเอกสารชั้นความลับ 1–3 เข้าระบบ
> จนกว่าจะปิดงาน P3** สิทธิ์ `confidential.access` ยังบังคับที่ชั้นแอปตามปกติ แต่ป้องกันคนที่เข้าถึงดิสก์ไม่ได้
...
ข้อ 1–3, 6, 7 ทำแล้วใน P2 · ข้อ 4–5 รอ P3 ตาม D18
