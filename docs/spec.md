...
17. [ภาคผนวก ข — การเข้าสู่ระบบ: Better Auth · Google Sign-In](#17-ภาคผนวก-ข--การเข้าสู่ระบบ-เทียบ-better-auth--เพิ่ม-google-sign-in--ข้อเสนอแนะ)
...
| **D19** | Authentication *(28 ส.ค. 2569)* | **คงระบบ auth ที่เขียนเองไว้ ไม่ย้ายไป Better Auth** — เหตุผล ราคาที่เทียบแล้ว และเงื่อนไขที่จะกลับมาทบทวนอยู่ที่ §17 · เพิ่ม **Google Sign-In** เป็น identity provider ตัวที่สองบนของเดิม (§17.3) · **ไม่รับ LINE** เพราะจำกัดโดเมนองค์กรไม่ได้ · **ล็อกอินด้วยรหัสผ่านยังเป็นทางหลักเสมอ** เพราะ D13 deploy on-premise และ Google ต้องออกอินเทอร์เน็ต |
...
- SSO (LDAP/AD/Google Workspace/M365), 2FA — *นอก MVP ตาม D5 · การเปิด **Google Sign-In** เป็นทางเลือกเพิ่ม วิเคราะห์ไว้แล้วที่ §17 รอเคาะที่ §15 ข้อ 8*
...
Session             id, userId, activeOrgUnitId, authMethod, ip, userAgent,
...
| 5 | ~~**ปีที่ใช้รีเซ็ตเลข**~~ | ✅ **ตอบครบแล้ว 26 ส.ค. 2569** — หน่วยงานยืนยันว่าใช้ **ปีปฏิทิน** (1 ม.ค. – 31 ธ.ค.) ไม่ใช่ปีงบประมาณ · ตรงกับค่า `CALENDAR` ที่ตั้งไว้อยู่แล้ว **ไม่ต้องแก้อะไร** · เส้นตาย 1 ต.ค. 2569 จึงไม่มีผลอีกต่อไป |
...
| 8 | **การเข้าสู่ระบบด้วย Google** *(ตั้งคำถาม 28 ส.ค. 2569)* — ~~(ก) รับ Google เป็นทางเลือกเพิ่ม หรือย้ายไป Better Auth ทั้งระบบ~~ ✅ **ตอบแล้ว 28 ส.ค. 2569 — ไม่ย้าย library · รับ Google เป็นทางเลือกเพิ่ม ดู D19** · ~~(ค) ผู้ใช้ที่ผูก Google แล้วยังล็อกอินด้วยรหัสผ่านได้อยู่หรือไม่~~ ✅ **ได้อยู่ — รหัสผ่านเป็นทางหลัก** · **ยังค้าง: (ข)** ยืนยันว่าจำกัดเฉพาะโดเมน `krirk.ac.th` (ตั้งค่าไว้แล้วที่ `GOOGLE_ALLOWED_HD` เปลี่ยนได้โดยไม่ต้องแก้โค้ด) · **(ง)** ผู้ที่เข้าถึงเอกสารชั้น 2–3 ต้องบังคับ 2FA ด้วยหรือไม่ | ข้อ (ข) ถ้าไม่จำกัดโดเมน = เปิดให้บัญชีนอกองค์กรเข้าระบบสารบรรณ · ข้อ (ง) ถ้าตอบว่าต้องมี ให้ทบทวน D19 ใหม่ตามเงื่อนไขใน §17.4 |
...
## 17. ภาคผนวก ข — การเข้าสู่ระบบ: เทียบ Better Auth · เพิ่ม Google Sign-In · ข้อเสนอแนะ

*บันทึก 28 ส.ค. 2569 — ตอบสามคำถามที่ผู้ดูแลตั้งไว้ · **ยังไม่ใช่มติ** ส่วนที่ต้องเคาะอยู่ที่ §15 ข้อ 8*

### 17.1 ของที่มีอยู่จริงตอนนี้

Custom auth ที่เขียนเอง **~1,387 บรรทัด** ปิดงานไปตั้งแต่ P1 และมีเทสต์คุมอยู่

| ไฟล์ | บรรทัด | หน้าที่ |
|------|--------|---------|
| `src/lib/auth/session.ts` | 205 | JWT (`jose` HS256 + issuer/audience) ใน cookie `esaraban-session` **คู่กับแถวในตาราง `Session`** |
| `src/lib/auth/password.ts` | 152 | Argon2id (`@node-rs/argon2` · m=19MiB t=2 p=1 ตาม OWASP) · นโยบายรหัสผ่าน · รหัสผ่านชั่วคราว |
| `src/lib/auth/rate-limit.ts` | 107 | sliding window ต่อ IP (ในหน่วยความจำ) |
| `src/lib/auth/username.ts` | 37 | สร้างชื่อผู้ใช้จากอีเมล (D17) |
| `src/server/services/auth.service.ts` | 517 | login · logout · เปลี่ยนรหัสผ่าน · สลับหน่วยงาน · คำขอสมัคร · คำขอ/รีเซ็ตรหัสผ่าน |
| `src/server/actions/auth.actions.ts` + `src/schemas/auth.schema.ts` | 220 | Server Action + Zod |

คุณสมบัติที่ได้มาแล้วและ**ผูกกับกติกาของงานสารบรรณ ไม่ใช่ของทั่วไป**:

- **revoke ได้จริง** — cookie เป็น JWT แต่ทุก request ตรวจแถว `Session` ด้วย · ระงับบัญชีแล้วเตะออกทันที
- **idle 30 นาที + absolute 8 ชม.** (2 ชม. ถ้าไม่ติ๊ก "จดจำ") · เขียน `lastSeenAt` ถี่สุด 1 ครั้ง/นาที
- **`Session.activeOrgUnitId`** — Context Switcher ของผู้ใช้หลายสังกัด (§5.2) อยู่ในแถวเซสชัน
- **ล็อกบัญชีแบบ exponential backoff** (`failedLoginCount` + `lockedUntil` ในฐาน) **คู่กับ** rate limit ต่อ IP
- **ไม่บอกผู้โจมตีว่าอะไรผิด** — ชื่อผู้ใช้ไม่มีกับรหัสผ่านผิดตอบข้อความเดียวกัน และ hash ทิ้งหนึ่งครั้งให้เวลาตอบใกล้เคียงกัน
- **ทุกเหตุการณ์ลง audit ที่มี hash chain** (§8.5) พร้อม `sessionId` · `ip` · `userAgent`
- **สมัครสมาชิกเป็นคิวให้ผู้ดูแลอนุมัติ** (`RegistrationRequest`) ไม่ใช่สมัครแล้วเข้าได้เลย
- **ลืมรหัสผ่านเป็นคิวให้ผู้ดูแลรีเซ็ต** (D10 — MVP ไม่มีอีเมลขาออก)

### 17.2 เทียบกับ Better Auth

Better Auth คือ auth framework ของ TypeScript ที่ self-host เองทั้งหมด (เวอร์ชันเสถียร v1.7)
รองรับ Prisma adapter · เซสชันเป็นแบบเก็บในฐานข้อมูลคู่กับ cookie · ขยายด้วยระบบ plugin

| หัวข้อ | ของเราตอนนี้ | Better Auth | ต่างกันจริงไหม |
|--------|--------------|-------------|----------------|
| รูปแบบเซสชัน | JWT + แถว `Session` (revoke ได้) | token ในฐาน + cookie · มี cookie cache / stateless ให้เลือก | **แทบไม่ต่าง** — เราได้ผลเดียวกันอยู่แล้ว |
| revoke | `revokedAt` + `revokeAllSessions()` | revoke ทีละอัน / ทั้งหมด / อัตโนมัติตอนเปลี่ยนรหัสผ่าน | เท่ากัน |
| อายุเซสชัน | idle 30 น. + absolute 8 ชม. | `expiresIn` ปริยาย 7 วัน + `updateAge` ต่ออายุอัตโนมัติ + `freshAge` | **ของเราเข้มกว่า** · ของเขาต้องตั้งเองให้ตรงกติกา §8.4 |
| แฮชรหัสผ่าน | Argon2id | scrypt (ปริยาย) · เปลี่ยนเป็น argon2 ได้ผ่าน `password.hash/verify` | เท่ากันได้ ต้องตั้งเอง |
| ล็อกบัญชี | exponential backoff ต่อบัญชี เก็บในฐาน | มี rate limit ในตัว (ปริยายเก็บในหน่วยความจำเช่นกัน) | **ของเราเข้มกว่าและอยู่ข้าม instance** |
| Social login | ไม่มี | มีให้ครบ รวม Google + ตัวเลือก `hd` จำกัดโดเมน Workspace | **ข้อได้เปรียบจริงข้อที่หนึ่ง** |
| 2FA / passkey / magic link | ไม่มี | plugin พร้อมใช้ | **ข้อได้เปรียบจริงข้อที่สอง** |
| SSO / OIDC / M365 | ไม่มี | plugin `sso` · เป็น OIDC provider เองได้ | **ข้อได้เปรียบจริงข้อที่สาม** |
| multi-tenant | `tenantId` ทุกตาราง (D1) | plugin `organization` / multi-tenancy | ของเราทำตามโดเมนตัวเองอยู่แล้ว |
| audit hash chain | เขียนเองทุกจุด | ไม่มี ต้องเขียน hook เอง | **ของเรามี ของเขาต้องทำเพิ่ม** |
| Context Switcher หลายสังกัด | อยู่ใน `Session.activeOrgUnitId` | ไม่มีแนวคิดนี้ ต้องต่อเองด้วย `additionalFields` | **ของเรามี ของเขาต้องทำเพิ่ม** |
| ชั้นความลับ / `clearanceLevel` | อยู่ใน `User` + `can()` | ไม่มี เป็นเรื่องของแอป | เท่ากัน (ต้องเขียนเองทั้งคู่) |

**ราคาที่ต้องจ่ายถ้าย้าย** — ไม่ใช่ "ลงแพ็กเกจแล้วเสร็จ" เพราะ schema ของ Better Auth
ชนกับ schema ของเราหลายจุด และจุดที่ชนล้วนเป็นจุดที่มีข้อมูลจริงอยู่แล้ว

1. **`User.email` ของเราเป็น `String?` และไม่ unique** · Better Auth บังคับ `email` ไม่ว่างและไม่ซ้ำ → ต้อง migrate และไล่ dedupe ข้อมูลจริงก่อน
2. **ตัวระบุหลักของเราคือ `username`** (D17) ไม่ใช่อีเมล → ต้องใช้ plugin `username` และยอมรับว่าโครงคนละแบบ
3. **`User.passwordHash` เป็น NOT NULL** · Better Auth เก็บรหัสผ่านไว้ในตาราง `account` แยกออกไป → ต้องย้ายข้อมูลของผู้ใช้ทุกคน
4. **`Session` ของเรามี `activeOrgUnitId` ที่เป็น FK ไป `OrgUnit`** และ `AuditLog.sessionId` อ้างถึงเซสชัน → เปลี่ยนตารางเซสชันแปลว่าต้องแตะ audit ที่เป็น append-only + hash chain ซึ่ง **แก้ย้อนหลังไม่ได้ตามที่ออกแบบไว้**
5. **สมัคร = คิวรออนุมัติ** ไม่ใช่ `signUp` ที่เข้าได้ทันที → ต้องปิดหรือครอบ endpoint สมัครของ Better Auth ไม่ให้เปิดทางเข้าที่ข้ามการอนุมัติ
6. **ลืมรหัสผ่าน = คิวให้ผู้ดูแลรีเซ็ต** เพราะ MVP ไม่มีอีเมลขาออก (D10) → ฟีเจอร์ฝั่งอีเมลของ Better Auth ใช้ไม่ได้จนกว่าจะมี SMTP
7. **เทสต์ 166 unit + 124 integration ที่เขียวอยู่ตอนนี้ครอบเส้นทาง auth เดิม** — ย้าย library แปลว่าเขียนใหม่ทั้งชุด

⚠️ **ยังไม่ได้ทดสอบว่า Better Auth v1.7 ทำงานกับ Next 16.3.1 (ที่เปลี่ยนชื่อ `middleware` เป็น `proxy`) และ Prisma 7.9 ได้จริง** — ถ้าจะเอาจริงต้อง spike แยกก่อน ห้ามถือว่าผ่าน

### 17.3 Google Sign-In — ออกแบบให้ต่อกับ auth ปัจจุบันได้เลย

**ข้อสรุปเชิงเทคนิค: การรับ Google ไม่จำเป็นต้องเปลี่ยน library** · Google ทำหน้าที่แค่
"พิสูจน์ว่าเป็นใคร" ส่วนเซสชัน · สิทธิ์ · ชั้นความลับ · audit ยังเป็นของระบบเราทั้งหมด

**ขั้นตอน**

1. ผู้ใช้กด "เข้าสู่ระบบด้วย Google" → redirect ไป Google พร้อม `state` + PKCE + `nonce` (เก็บ `state` ไว้ใน cookie ที่ลงนามแล้วอายุสั้น)
2. Google ส่งกลับที่ `/api/auth/callback/google` → แลก code เป็น token → อ่าน `sub` · `email` · `email_verified` · `hd`
3. **ด่านตรวจสามชั้นก่อนยอมให้เข้า**
   - `email_verified === true`
   - `hd === "krirk.ac.th"` (โดเมน Google Workspace ขององค์กร) — บัญชี gmail ส่วนตัวเข้าไม่ได้
   - **มีบัญชีในระบบอยู่แล้ว** และ `isActive = true` · `deletedAt = null`
4. ผูกบัญชี: ครั้งแรกจับคู่ด้วยอีเมล แล้ว **เก็บ `sub` ของ Google ไว้ใช้ครั้งต่อไป**
5. สร้างเซสชันด้วย `createSession()` ตัวเดิม → ทุกอย่างหลังจากนี้เหมือน login ปกติทุกประการ

⚠️ **ห้าม auto-provision ผู้ใช้ใหม่จาก Google เด็ดขาด** — ผู้ใช้ที่ไม่มีหน่วยงาน ไม่มีบทบาท
และไม่มี `clearanceLevel` ไม่มีความหมายในระบบนี้ · และถ้าเปิดไว้ ใครก็ตามที่มีบัญชีในโดเมน
มหาวิทยาลัย (รวมนักศึกษา) จะเดินเข้าระบบสารบรรณได้ทันที · คนใหม่ต้องผ่านคิวอนุมัติเหมือนเดิม

⚠️ **หลังผูกครั้งแรกแล้วให้จับคู่ด้วย `sub` ไม่ใช่อีเมล** — Workspace เปลี่ยนชื่ออีเมลได้
และอีเมลของคนที่ลาออกถูกยกให้คนใหม่ได้ · ยึดอีเมลอย่างเดียวคือเปิดทางให้เข้าบัญชีผิดคน

**สิ่งที่ต้องแก้ในฐานข้อมูล**

| แก้อะไร | ทำไม |
|---------|------|
| ตารางใหม่ `UserIdentity` (`userId` · `provider` · `providerAccountId` · `email` ตอนผูก · `linkedAt` · `lastUsedAt` · unique `[provider, providerAccountId]`) | เก็บ `sub` ของ Google · รองรับ provider อื่นในอนาคตโดยไม่ต้องแก้ `User` อีก |
| `User.email` → **บังคับ lowercase + unique ต่อ tenant** | ตอนนี้ซ้ำกันได้ ทำให้การจับคู่ครั้งแรกไม่ปลอดภัย · ต้องไล่ dedupe ข้อมูลจริงก่อน migrate |
| `User.passwordHash` → **nullable** | ผู้ใช้ที่เข้าด้วย Google อย่างเดียวไม่มีรหัสผ่าน · ปัจจุบันคอลัมน์นี้เป็น NOT NULL |
| `AUDIT_ACTIONS` เพิ่ม `auth.login.google` · `auth.identity.linked` · `auth.identity.unlinked` | §8.5 ต้องตามได้ว่าใครเข้าระบบด้วยวิธีใด |

**ของเดิมที่ใช้ไม่ได้กับเส้นทาง Google และต้องตัดสินใจ**

- `failedLoginCount` / `lockedUntil` ไม่มีความหมาย (ไม่มีรหัสผ่านให้เดา) — แต่ rate limit ต่อ IP ยังต้องมีที่ callback
- `mustChangePassword` ข้ามได้เลย · หน้า "ลืมรหัสผ่าน" ไม่เกี่ยวกับผู้ใช้กลุ่มนี้
- ผู้ใช้ที่ผูก Google แล้ว **ยังล็อกอินด้วยรหัสผ่านได้อยู่** ถ้ายังมี hash — ถ้าไม่ต้องการต้องปิดเป็นนโยบาย (ดู §15 ข้อ 8 (ค))

**เงื่อนไขระบบที่ต้องมีก่อน — ข้อนี้สำคัญกว่าที่คิด**

⚠️ **D13 กำหนดว่า deploy on-premise** · Google Sign-In บังคับให้ **เซิร์ฟเวอร์ต้องออกอินเทอร์เน็ตได้**
(แลก token กับ `oauth2.googleapis.com`) และผู้ใช้ต้องเข้า `accounts.google.com` ได้
→ ถ้าเครื่องอยู่ในวงปิด ฟีเจอร์นี้ใช้ไม่ได้เลย · **ต้องคงล็อกอินด้วยรหัสผ่านไว้เป็นทางหลักเสมอ**
ไม่ใช่แค่ทางสำรอง — วันที่อินเทอร์เน็ตขาด งานสารบรรณต้องเดินต่อได้

ต้องมีเพิ่ม: โดเมนสาธารณะ + **TLS certificate** (ยังค้างอยู่ที่ §15 ข้อ 7) เพราะ redirect URI
ของ Google ต้องเป็น https · nginx ต้องส่ง `/api/auth/callback/google` เข้าแอป ·
CSP ปัจจุบันไม่ต้องแก้ (การ redirect ไม่นับเป็น `form-action` และเราไม่ใช้ Google One Tap
ที่ต้องฝัง script ข้ามโดเมน — ถ้าจะใช้ One Tap ต้องผ่อน `script-src` ซึ่ง **ไม่แนะนำ**)

### 17.4 ความเห็น — ควรคงของเดิมไว้ แล้วต่อ Google เพิ่มบนของเดิม

**ข้อเสนอ: ยังไม่เปลี่ยนไป Better Auth · เพิ่ม Google เป็น identity provider ตัวที่สอง**

เหตุผล

1. **สิ่งที่ Better Auth ให้เปล่า เราสร้างไปแล้วเกือบหมด** — เซสชันที่ revoke ได้ · ล็อกบัญชี · rate limit · นโยบายรหัสผ่าน · Argon2id · เทสต์ที่เขียวอยู่ · ที่เหลือคือ social/2FA/SSO ซึ่ง **ข้อที่ต้องการจริงตอนนี้มีข้อเดียวคือ Google** และข้อนั้นทำบนของเดิมได้ในเวลาที่สั้นกว่ามาก
2. **ราคาของการย้ายไม่ได้อยู่ที่ auth แต่อยู่ที่ audit** — ตาราง audit เป็น append-only + hash chain ที่ตั้งใจให้แก้ย้อนหลังไม่ได้ · การเปลี่ยนตารางเซสชันที่ audit อ้างถึง คือความเสี่ยงที่ไม่ได้แลกกับฟีเจอร์ที่ใช้จริงวันนี้
3. **ระบบเพิ่งปิด P5 และกำลังจะเข้า UAT** — เปลี่ยนชั้น auth ตอนนี้คือรื้อพื้นใต้เท้าตัวเองก่อนให้ผู้ใช้จริงมาทดสอบ

**ประมาณการ**

| งาน | แรง | ความเสี่ยง |
|-----|-----|------------|
| ต่อ Google บน auth เดิม | ~2–4 วัน (ตาราง `UserIdentity` + callback + ปุ่ม + เทสต์) | ต่ำ — ของเดิมไม่ถูกแตะ |
| ย้ายไป Better Auth ทั้งระบบ | ~2–3 สัปดาห์ + spike ตรวจความเข้ากันได้ก่อน | **สูง** — migrate ข้อมูลผู้ใช้จริง · เขียนเทสต์ auth ใหม่ทั้งชุด · แตะ audit |

**เงื่อนไขที่จะทำให้กลับมาคิดใหม่ (ทบทวนทุกครั้งที่ข้อใดข้อหนึ่งเป็นจริง)**

- ต้องมี **2FA/passkey บังคับ** สำหรับผู้ที่เข้าถึงเอกสารชั้น 2–3 — ข้อนี้มีโอกาสเกิดจริงที่สุด (เขียน TOTP เองราว 300 บรรทัด เทียบกับได้ฟรีจาก plugin)
- ต้องต่อ **SSO กับ M365 / LDAP / AD** ของมหาวิทยาลัย ไม่ใช่แค่ Google
- **multi-tenant จริง** ตาม D1 (ตอนนี้ยังเป็น single-tenant ที่เผื่อ `tenantId` ไว้)
- ต้องเป็น **OIDC provider** ให้ระบบอื่นในมหาวิทยาลัยมาล็อกอินผ่าน

**สิ่งที่ต้องทำเผื่ออนาคตไว้ตั้งแต่ตอนนี้ ไม่ว่าจะเลือกทางไหน**

1. **กันเขตของ provider ให้ชัด** — โค้ด Google ต้องอยู่ใต้ `src/lib/auth/providers/` และคุยกับระบบผ่านสัญญาเดียวคือ "ยืนยันตัวตนแล้วได้ `userId`" · ห้ามให้โค้ดฝั่ง Google รู้จัก `Session` · `can()` · `clearanceLevel` โดยตรง · ทำแบบนี้แล้ววันที่ย้าย library จริง จุดที่ต้องแก้จะจำกัดอยู่ในโฟลเดอร์เดียว
2. **`UserIdentity` ออกแบบให้รองรับหลาย provider ตั้งแต่แรก** (`provider` เป็นคอลัมน์ ไม่ใช่ตั้งชื่อตารางว่า `GoogleAccount`) — วันที่เพิ่ม M365 จะไม่ต้อง migrate อีกรอบ
3. **rate limit ต้องย้ายไป Redis ก่อนขึ้นหลาย instance** — ข้อจำกัดนี้เป็นของทั้งสองทาง (Better Auth ปริยายก็เก็บในหน่วยความจำเหมือนกัน) และเป็นคนละเรื่องกับการเลือก library

### 17.5 แบบหน้า `/login` ที่เคาะแล้ว (`docs/sample_v8.png` · 28 ส.ค. 2569)

ผู้ดูแลส่งภาพตัวอย่างมาและเคาะสี่ข้อ · **ภาพเป็นแบบของ "การจัดวาง" ไม่ใช่แบบของสี** —
สีและฟอนต์ยังใช้ระบบเดิมของแอป (เขียวแบรนด์ `#3F6133` · แปดขั้นตัวอักษรตาม §23.17 ของ `progress.md`)
และแผงแบรนด์ด้านซ้าย (`AuthBrandPanel`) ยังอยู่เหมือนเดิม ภาพตัวอย่างครอบเฉพาะคอลัมน์ฟอร์ม

**ลำดับที่เคาะแล้ว** — บนลงล่าง

1. หัวข้อ + คำอธิบาย (ของเดิม)
2. **ปุ่ม "เข้าสู่ระบบด้วย Google"** เต็มความกว้าง
3. เส้นคั่นคำว่า **"หรือ"**
4. ช่องแรก (อีเมลหรือชื่อผู้ใช้) · ช่องรหัสผ่าน + ลิงก์ "ลืมรหัสผ่าน?" ที่หัวช่อง + ปุ่มลูกตา
5. ช่อง "จดจำการเข้าสู่ระบบ"
6. ปุ่ม "เข้าสู่ระบบ"
7. ท้ายหน้า "ยังไม่มีบัญชี? สมัครสมาชิก"

**สี่ข้อที่ตัดสินใจต่างจากภาพ และเหตุผล**

| เรื่อง | ภาพตัวอย่าง | ที่เคาะจริง | ทำไม |
|--------|-------------|-------------|------|
| ปุ่ม LINE | มี | **ไม่เอา** | LINE จำกัดโดเมนองค์กรไม่ได้เหมือน Google ที่มี `hd` — ใครมีบัญชี LINE ก็กดได้หมด · และ LINE คืนอีเมลให้ต่อเมื่อขอสิทธิ์พิเศษผ่านการอนุมัติ ทำให้จับคู่กับบัญชีในระบบไม่ได้เลยถ้าไม่ได้สิทธิ์นั้น · ไม่เหมาะกับระบบที่มีเอกสารชั้นความลับ |
| ป้ายช่องแรก | `Email` | **"อีเมลหรือชื่อผู้ใช้"** | D17 ให้ล็อกอินได้ทั้งสองอย่าง · เขียนว่า Email อย่างเดียวจะทำให้คนที่ระบบสร้างชื่อผู้ใช้ให้คิดว่าตัวเองเข้าไม่ได้ · หลังบ้านไม่ต้องแก้ |
| จดจำการเข้าสู่ระบบ | ไม่มี | **เก็บไว้** | §8.4 ผูกช่องนี้กับอายุเซสชัน (ติ๊ก = 8 ชม. · ไม่ติ๊ก = 2 ชม.) · ตัดออกแปลว่าต้องเลือกค่าเดียวให้ทุกคน ซึ่งลดความปลอดภัยของเครื่องที่ใช้ร่วมกันในสำนักงาน |
| สีปุ่มหลัก | ม่วง | **เขียวแบรนด์เดิม** | ภาพเป็นแบบจัดวาง ไม่ใช่แบบสี · ทั้งระบบใช้ชุดสีเดียวกันอยู่แล้ว |

⚠️ **ปุ่ม Google จะไม่แสดงเลยถ้ายังไม่ได้ตั้ง `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET`**
ตั้งใจให้เป็นแบบนี้ — ปุ่มที่กดแล้วไม่เกิดอะไรแย่กว่าไม่มีปุ่ม และทำให้ deploy ที่อยู่ในวงปิด
(ซึ่ง Google ใช้ไม่ได้ตาม §17.3) ได้หน้าจอที่ถูกต้องโดยไม่ต้องแก้โค้ด

### 17.6 สิ่งที่ทำจริง และสองเรื่องที่เจอตอนลงมือ (28 ส.ค. 2569)

ทำครบตาม §17.3 และ §17.5 แล้ว · ระหว่างทางเจอสองเรื่องที่แบบร่างมองไม่เห็น
ทั้งคู่มาจากคำถามเดียวว่า "บัญชีที่สมัครไว้แล้วมีอีเมลตรงกับ Google จะมีปัญหาไหม"

**เรื่องที่ 1 — `mustChangePassword` ทำให้ล็อกอินสำเร็จแล้วใช้งานไม่ได้**

`requireSession()` เด้งทุกหน้าไป `/change-password` เมื่อ `mustChangePassword = true`
และฟอร์มนั้นบังคับกรอก **รหัสผ่านปัจจุบัน** · ทั้ง `createUser()` และ `approveRegistration()`
ตั้งค่านี้เป็น `true` เสมอ → คนที่ผู้ดูแลเพิ่งสร้างบัญชีให้แล้วบอกว่า "ใช้ Google เข้าได้เลย"
จะเข้า Google ผ่าน แต่ไปติดอยู่หน้าที่กรอกอะไรไม่ได้ **ออกไปไหนไม่ได้เลย**

แก้ด้วยการเพิ่มคอลัมน์ **`Session.authMethod`** (`PASSWORD` | `GOOGLE`) แล้วให้ `requireSession()`
ข้ามด่านนี้เฉพาะเซสชันที่เข้ามาด้วย Google

- จุดประสงค์ของ §8.4 ข้อนี้คือกันไม่ให้ใช้รหัสผ่านชั่วคราวที่คนอื่นรู้ต่อไปเรื่อย ๆ
  ซึ่งคนที่เข้าด้วย Google ไม่ได้ใช้รหัสผ่านนั้นอยู่แล้ว
- **ค่าในตาราง `users` ยังเป็น `true` เหมือนเดิม** — วันที่เขากลับมาล็อกอินด้วยรหัสผ่าน
  จะโดนบังคับเปลี่ยนตามปกติ · ไม่ได้ยกเลิกกติกา แค่ไม่บังคับใช้กับทางที่ไม่เกี่ยวข้อง

**เรื่องที่ 2 — จับคู่อีเมลข้าม tenant ได้ (ยังไม่กระทบวันนี้ แต่ปิดไปแล้ว)**

การหาผู้ใช้จากอีเมลเดิมไม่ได้กรอง `tenantId` ขณะที่ unique ของ `users` เป็น `[tenantId, email]`
วันที่ D1 กลายเป็น multi-tenant จริง อีเมลเดียวกันจะมีได้หลายแถวแล้ว `findFirst` จะหยิบตัวไหนก็ได้
= **ล็อกอินเข้าบัญชีผิดคนข้ามหน่วยงาน** · ตอนนี้กรองด้วย tenant เสมอ และแยกเป็นฟังก์ชัน
`resolveTenantIdForGoogle()` ให้วันที่มีหลาย tenant จริงแก้ที่เดียว — แมปจาก `hostedDomain` ไปเป็น tenant

⚠️ **ห้ามส่ง `tenantId` ที่เป็น `undefined` เข้า `where` ของ Prisma** — มันจะเมินตัวกรองทิ้ง
ทั้งอันเงียบ ๆ แล้วกลับไปค้นข้ามทุก tenant ซึ่งคือบั๊กเดิมที่เพิ่งปิดไป

**เรื่องที่ตรวจแล้วไม่มีปัญหา**

| ตรวจอะไร | ผล |
| --- | --- |
| สิทธิ์เปลี่ยนไปไหม | ไม่ — `can()` ตัดสินจากบทบาท สังกัด และ `clearanceLevel` ไม่มีจุดไหนแตะอีเมล |
| สมัครซ้ำด้วยอีเมลที่มีบัญชีแล้ว | `submitRegistration()` บล็อกอยู่แล้วทั้งกรณีมีผู้ใช้และมีคำขอค้าง |
| ผู้ดูแลแก้อีเมลของคนที่ผูก Google แล้ว | ยังล็อกอินได้ เพราะจับคู่ด้วย `sub` ไม่ใช่อีเมล |
| อนุมัติคำขอที่อีเมลชนกับผู้ใช้เดิม | `assertEmailAvailable()` ตอบเป็นข้อความไทย ไม่ปล่อยให้ชน unique index แล้วโยน P2002 ดิบ |

**ของที่ทำเสร็จแล้ว**

| ส่วน | ไฟล์ |
| --- | --- |
| กลไก OAuth/OIDC | `src/lib/auth/providers/google.ts` · `oauth-state.ts` |
| เส้นทาง | `/api/auth/google/start` · `/api/auth/callback/google` |
| ด่านตรวจ + ผูกบัญชี + เซสชัน | `loginWithGoogle()` ใน `src/server/services/auth.service.ts` |
| หน้าจอ | `src/components/auth/google-button.tsx` + `login-form.tsx` + `(auth)/login/page.tsx` |
| ฐานข้อมูล | `user_identities` · `users.email` unique ต่อ tenant · `users.passwordHash` ว่างได้ · `sessions.authMethod` |
| เทสต์ | unit 180 (+14) · integration 134 (+10) · e2e 114 (+3) |

**ยังไม่ได้ทำ (รอคำตอบ §15 ข้อ 8 ง หรือรอความต้องการจริง)**

- ถอนการผูกบัญชี Google — `auth.identity.unlinked` มีรหัส audit รออยู่แล้ว แต่ยังไม่มีปุ่มให้ผู้ใช้กด
- หน้า "ตั้งรหัสผ่าน" สำหรับผู้ใช้ที่ไม่มีรหัสผ่านเลย — จำเป็นก็ต่อเมื่อมีบัญชีที่ตั้งใจให้เข้าด้วย Google อย่างเดียว
- บังคับ 2FA สำหรับผู้เข้าถึงเอกสารชั้น 2–3
