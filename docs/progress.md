...
3. ~~**Responsive polish**~~ — **เสร็จแล้ว (§23.18)** · ตรวจจริง 18 หน้า × 4 ความกว้าง
   แล้วพบว่า `/documents/new` พังจริงทุกจอที่แคบกว่าราว 900px — แก้แล้ว
...
### 23.16 ⚠️ ผลรอบแรกของ CI — แดงทั้งสามงาน และทั้งสามเป็นบั๊กจริง

รอบแรก (`54d4faf` · run #1) แดงหมดทั้งสามงาน · ตัว workflow ทำงานถูกทุกขั้น
(pnpm install 15-17 วิ · migrate 2 วิ · seed 3 วิ · service Postgres ขึ้นปกติ)
สิ่งที่แดงคือของที่เขียวมาตลอดบนเครื่องพัฒนาแต่ไม่เคยเขียวบนฐานที่เกิดใหม่

**บทเรียนรวบยอด: ชุด integration กับ e2e ไม่เคยรันผ่านบนฐานที่ seed สดได้จริง**
ที่ผ่านมาเขียวเพราะฐาน dev สะสมข้อมูลและการแก้ชั้นความลับด้วยมือไว้ตั้งแต่ตอนทดสอบ P3
บันทึกเดิมที่ว่า "118 เคสเขียว / 19 เคสเขียว" จึงจริงเฉพาะบนเครื่องเรา

#### ข้อ 1 ✅ typecheck แดง 17 error — `PageProps` ไม่มีตัวตนใน clone ใหม่

```
src/app/(app)/documents/[id]/page.tsx(66,62): error TS2304: Cannot find name 'PageProps'.
src/app/(app)/layout.tsx(12,55): error TS2304: Cannot find name 'LayoutProps'.
src/app/api/files/[attachmentId]/route.ts(22,15): error TS2304: Cannot find name 'RouteContext'.
```

`PageProps` / `LayoutProps` / `RouteContext` เป็น **global type ที่ Next generate ให้**
ลง `.next/types` ตอน `next dev` หรือ `next build` พร้อมกับ `next-env.d.ts` ·
ทั้งสองอย่างอยู่ใน `.gitignore` (Next แนะนำเองในเอกสารว่าให้ ignore) เครื่องเราจึงมี
เพราะเคยรัน dev แต่ **clone ใหม่ไม่มี** แล้ว `tsc --noEmit` เปล่า ๆ พังทันที

แก้ด้วยสูตรที่เอกสารของ Next รุ่นนี้เขียนไว้ตรง ๆ สำหรับ CI
...
ใส่ไว้ที่ **สคริปต์ใน `package.json` ไม่ใช่ที่ workflow** เพราะ pre-commit hook
เรียกสคริปต์เดียวกัน — ถ้าแก้แค่ฝั่ง CI คนที่เพิ่ง clone มาจะเจอ hook พังแบบไม่มีคำอธิบาย
...
```
Error: ชั้นความลับบนฐานไม่ตรงกับที่ชุดเทสต์นี้ต้องการ (registrar=2 · rattana.wong=1 · somchai.j=0)
ServiceError: นายทะเบียนหนังสือลับของหน่วยงานนี้มีชั้นความลับไม่ถึงระดับ 2 ...: นางรัตนา วงศ์ประเสริฐ (ชั้น 1)
```

ฐานที่ seed สดให้ `rattana.wong` ชั้น 1 แต่ชุดเทสต์ต้องการ ≥ 2 ·
`confidential-registrar.test.ts` มีด่านเช็กแล้วโยน error ยกไฟล์ (12 เคสจึงขึ้น skipped)
...
30 ผ่าน 1 ตก (จาก 31 เคส · ชุดโตจาก 19 เคสตอน §23.1 แล้ว)

```
report-export.spec.ts:82 › ค้นภาษาไทยจากคำกลางประโยคได้ (§9.2 — pg_trgm)
  getByText(/อบรม/) — element(s) not found
```

เคสนี้ค้นคำว่า "อบรม" แล้วคาดว่าจะเจอเอกสาร แต่คำนี้มีอยู่ในเอกสารบนฐาน dev เท่านั้น ·
fixture ของ e2e ไม่ได้สร้างให้ หน้าค้นหาจึงตอบ "พบ 0 ฉบับ" ซึ่งถูกต้องตามข้อมูลที่มี

**แก้แล้ว:** `ensureE2ESearchDocument()` ใน `tests/e2e/fixtures/db-fixture.ts` สร้างเอกสาร
ชื่อเรื่อง `[e2e] โครงการอบรมการใช้งานระบบสารบรรณอิเล็กทรอนิกส์` ตอน globalSetup ·
คำนำหน้า `[e2e]` ทำให้ teardown เดิมตามลบได้โดยไม่ต้องแก้อะไรเพิ่ม · idempotent (มีแล้วไม่สร้างซ้ำ)

คำค้นกับชื่อเรื่องย้ายไปอยู่ที่ `fixtures/constants.ts` (`E2E_SEARCH_TERM` ·
`E2E_SEARCH_SUBJECT`) แล้วให้ spec ยืนยัน**ชื่อเรื่องของ fixture ตรง ๆ** แทน regex `/อบรม/`
ที่จะติดเอกสารอะไรก็ได้ที่บังเอิญมีคำนี้ — เคสจึงพิสูจน์สิ่งที่มันอ้างว่าพิสูจน์จริง ๆ

⚠️ **"อบรม" ต้องอยู่กลางคำเสมอ** ("โครงการ**อบรม**การ...") ไม่ใช่ต้นคำ — นั่นคือทั้งหมด
ที่เคสนี้มีไว้พิสูจน์ (§9.2 · pg_trgm ค้นคำกลางประโยคภาษาไทยได้ ต่างจาก full-text
ที่ตัดคำไทยไม่ได้) · ถ้าใครแก้ชื่อเรื่องให้ขึ้นต้นด้วย "อบรม" เคสจะยังเขียวแต่ไม่เหลือความหมาย

**ผล: 31/31 เขียวบนฐานที่ seed สด** (ใช้วิธีเดียวกับข้อ 2 แต่ตั้ง `DATABASE_URL` ให้ทั้ง
`pnpm test:e2e` — ตรวจแล้วว่าค่าจาก shell ชนะ `--env-file=.env` ของ globalSetup
และชนะ `.env` ที่ Next โหลดตอน `pnpm build && pnpm start` ด้วย จึงชี้ทั้งชุดไปฐานทดสอบได้จริง)

#### ผลรอบสอง — เขียวครบทั้งสามงาน

run [#2](https://github.com/rattanawong-it/e-saraban/actions/runs/33035612765) (`b61bcb0`)

| งาน | ผล | เวลา |
| --- | --- | --- |
| `checks` | ✅ | 46 วินาที |
| `integration` | ✅ 124/124 | 63 วินาที |
| `e2e` | ✅ 31/31 | 137 วินาที |

ทั้งแผงจบใน ~2.3 นาที (สามงานวิ่งขนานกัน) · ตั้งแต่นี้ไปทุก push ขึ้น `main` และทุก PR
มีด่านนี้รออยู่ — และที่สำคัญกว่าคือ **ชุดเทสต์ทั้งหมดพิสูจน์ตัวเองบนฐานที่เกิดใหม่ได้แล้ว
ไม่ใช่แค่บนเครื่องที่ฐานสะสมข้อมูลไว้**

---

### 23.17 รอบปรับ UI ตามที่ผู้ดูแลสั่ง (27 ส.ค. 2569 · `3a17692` → `3fb3470`)

รอบนี้ไม่ใช่ฟีเจอร์ใหม่ — เป็นการเก็บงานหน้าจอตามที่ผู้ดูแลตรวจแล้วสั่งกลับมา
พร้อมรูปตัวอย่างที่เขาส่งให้ (`docs/sample_*.png` · ⚠️ ยังไม่ได้ commit แต่โค้ดอ้างถึงอยู่)

**Type scale 8 ขั้นเป็นของจริงแล้ว** (`c827b7d`) — เดิมมีขนาดตัวอักษรตั้งเอง 18 ขนาด
กระจายอยู่ 38 ไฟล์ · ตอนนี้เหลือแปดขั้นที่ประกาศใน `globals.css`:
`--text-display` / `title-l` / `title` / `body` / `section` / `label` / `caption` / `micro`
**ห้ามเขียน `text-[13.5px]` หรือขนาดตั้งเองอีก** · ขนาดตั้งต้นของ `body` = 15px ·
ตั้งใจไม่ผูก line-height เข้ากับขั้น (ให้แต่ละที่คุมเอง)

#### ⚠️⚠️ กับดักคู่ที่เสียเวลาที่สุดของรอบนี้ — typecheck / eslint / integration / e2e จับไม่ได้สักตัว

**1. tailwind-merge ตัดคลาสสีทิ้ง** (`9600973`) — `cn()` ไม่รู้จัก `text-label` `text-caption`
จึงเดาว่าเป็น "สีข้อความ" แล้วให้มันไปทับ `text-white` ที่มาก่อน → ปุ่ม "สร้างหนังสือใหม่"
กลายเป็นเขียวเข้มบนเขียวเข้ม อ่านไม่ออก · แก้ที่ `src/lib/utils.ts` ด้วย `extendTailwindMerge`
ประกาศแปดขั้นเป็นกลุ่ม `font-size`
**เพิ่มขั้นใหม่เมื่อไหร่ต้องมาเติมที่นี่ด้วย** — มี unit test ล็อกไว้ใน `utils.test.ts`

**2. `--font-num` ต้องมี Anuphan ต่อท้าย Inter** (`166fde4`) — Inter ไม่มีอักษรไทย ·
ข้อความไทยที่อยู่ในคลาส `.tabular` (ใช้อยู่ 20 ไฟล์) จึงตกไปใช้ฟอนต์ของเครื่อง
เห็นชัดว่าเป็นคนละแบบกับทั้งระบบ · เบราว์เซอร์เลือกฟอนต์ **ทีละตัวอักษร**
ตัวเลขจึงยังได้ Inter ตามที่ตั้งใจ · ล็อกไว้ที่ `theme-tokens.test.ts`

บทเรียนร่วมของทั้งสองข้อ: **ความถูกต้องของงานหน้าจอไม่มีด่านอัตโนมัติไหนจับให้**
นอกจากจะเขียนเทสต์ที่เล็งไปที่ค่านั้นตรง ๆ — ทั้งคู่ตอนนี้มีเทสต์ล็อกแล้ว

#### สิ่งที่ผู้ดูแลเคาะในรอบนี้

- **หน้าภาพรวมจบในหน้าจอเดียว** (เฉพาะจอ `lg` ขึ้นไป) — `lg:h-[calc(100dvh-4.25rem-3.5rem)]`
  ⚠️ สูตรผูกกับ `h-17` ของ header และ `lg:py-7` ของ main **ด้วยตัวเลขที่พิมพ์เอง** ·
  แก้ความสูง header แล้วต้องมาแก้ที่นี่ · ทุกชั้น flex ถึงกล่องที่เลื่อนได้ต้องมี `min-h-0` ·
  e2e ล็อกสามขนาดจอไว้แล้ว
- **หน้าค้นหาเปิดครั้งแรกไม่มีแถบเลื่อน** (`3fb3470`) — `xl:grid-cols-4` ยุบตัวกรอง
  จาก 5 แถวเหลือ 4 · **ถอดออกหรือเพิ่มช่องกรองใหม่แล้วแถบเลื่อนกลับมาทันที**
  (ต้องการจอสูง ≥ 768px)
- **วันที่ที่แสดงเป็นข้อมูลใช้ วว/ดด/ปปปป พ.ศ.** ทั้งระบบ (`e12c0b1` — ค่าปริยายของ
  `formatThaiDate` เป็น `short` แล้ว)
- **ปล่อยรูปแบบช่อง `type=date` ไว้ตามเบราว์เซอร์** (`9f4f572`) — ผู้ดูแลเคาะเองว่าไม่ต้องดัด
  **อย่าไปแก้ถ้าไม่ได้ถูกสั่งใหม่**
- คำอธิบายของทุกหน้าย้ายไปอยู่บรรทัดเดียวกับหัวข้อ (`9f9bebb`)
- หน้าภาพรวมถอดหมวด "ผู้ใช้และหน่วยงาน" กับการ์ด "สังกัดและบทบาทของฉัน" ออก
  (`d95a5fe` — ลบ `getDashboardStats()` ที่ยิง COUNT 8 ครั้งทิ้งไปด้วย)
- ปุ่ม "ล้างเงื่อนไข" ผูกกับค่าตั้งต้นชุดเดียวที่ `src/constants/filters.ts` (`3a17692`)

#### ⚠️ dev server ของ Turbopack ไม่ generate Tailwind utility ตัวใหม่ให้ทันที

เจอจริงในรอบนี้ — เขียน `lg:grid-cols-2` ที่ไม่เคยมีใครใช้มาก่อน แล้วหน้าจอไม่เปลี่ยนเลย
ทั้งที่ markup ถูก · `sm:grid-cols-2` ที่มีอยู่แล้วทำงานปกติ จึงดูเหมือนโค้ดเราผิด ·
`touch globals.css` ก็ไม่ช่วย

**ตรวจงานดีไซน์ต้องดูบน production build เท่านั้น** — `pnpm build` แล้ว `PORT=3200 pnpm start`
(อย่าใช้ 3100 ซึ่งเป็นพอร์ตของ e2e) · ยืนยันได้ด้วยการ grep ไฟล์ CSS ใน
`.next/static/chunks/*.css` หรือฉีด element ทดสอบแล้วอ่าน `getComputedStyle` ·
ปิดเซิร์ฟเวอร์ด้วย `Get-NetTCPConnection -LocalPort 3200 | Stop-Process -Id $_.OwningProcess -Force`

### 23.18 ✅ Responsive polish — ปิดของค้างข้อสุดท้ายของ P5 ที่เป็นงานโค้ด (`000aca1` · 27 ส.ค. 2569)

ของค้างข้อนี้เคยถูกข้ามไปเพราะผู้ใช้บอกว่า "น่าจะเรียบร้อยแล้ว" (§23.14) ·
รอบนี้ตรวจจริงแล้วพบว่า **ไม่เรียบร้อย** — และหน้าที่พังคือหน้าที่ใช้บ่อยที่สุดหน้าหนึ่ง

#### ทำเป็นชุดเทสต์ที่วัดผลได้ ไม่ใช่ไล่ดูด้วยตา

`tests/e2e/responsive.spec.ts` เปิด **18 หน้า × 4 ความกว้าง = 72 เคส**
(+2 เคสของ setup = 74) แล้วยืนยันว่าหน้าไม่ล้นแนวนอน

| ความกว้าง | แทน |
| --- | --- |
| 390 × 844 | มือถือ |
| 768 × 1024 | แท็บเล็ตแนวตั้ง |
| 1024 × 768 | แล็ปท็อปเล็ก |
| 1440 × 900 | เดสก์ท็อป |

**ตารางที่กว้างเกินจอเป็นเรื่องปกติและต้องมีอยู่** — แต่มันต้องเลื่อนใน *กรอบของตัวเอง*
ไม่ใช่ดันทั้งหน้าให้เลื่อน · เคสจึงวัดที่ `documentElement.scrollWidth` ไม่ใช่ที่ตัวตาราง ·
และ `findOverflowingElements()` ข้าม element ที่มีบรรพบุรุษเป็นกรอบเลื่อนได้อยู่แล้ว

⚠️ **ตอนเคสแดงมันบอกด้วยว่า element ไหนเป็นตัวดัน** (tag · class · ขอบขวา · ข้อความ 30 ตัวแรก)
— ตั้งใจเขียนแบบนี้เพราะข้อความ `expected 851 to be <= 390` เฉย ๆ ไม่ช่วยใครเลย

#### เพิ่มบัญชี `e2e.admin` — เพราะห้าหน้าใต้ `/admin` สารบรรณกลางเปิดไม่ได้

`E2E_ADMIN_USERNAME` / `E2E_ADMIN_PASSWORD` ใน `fixtures/constants.ts` ·
`auth.setup.ts` ล็อกอินแยกแล้วเก็บเซสชันคนละไฟล์ (`state-admin.json`) ·
spec ใช้ผ่าน `test.use({ storageState })` เฉพาะกลุ่มหน้าผู้ดูแล
...
ถ้าไปเพิ่มบทบาทผู้ดูแลให้คนเดิมเพื่อความง่าย เคสนั้นจะแดงทันที

ปล่อยห้าหน้านี้ไม่ตรวจไม่ได้ เพราะเป็นหน้าที่มี**ตารางกว้างที่สุดในระบบ**
(เมทริกซ์สิทธิ์ · ผังหน่วยงาน 372 หน่วย · ตารางผู้ใช้)

#### ผลตรวจ: 17 หน้าผ่านหมด เหลือหน้าเดียวที่พัง — `/documents/new`

หน้าสร้างหนังสือกว้าง **851px ตายตัว** ไม่ว่าจอจะแคบแค่ไหน · แปลว่าพังตั้งแต่จอ
แคบกว่าราว 900px ลงมา — **รวมแท็บเล็ตกับแล็ปท็อปเล็กด้วย ไม่ใช่แค่มือถือ**

**ต้นเหตุเดียวกันหมดสามจุด: ความกว้างขั้นต่ำของลูก grid/flex เป็น `min-content` โดยปริยาย**
(`min-width: auto`) ซึ่งแปลว่า "ห้ามหดต่ำกว่าความกว้างขั้นต่ำของเนื้อหาข้างใน"

1. `xl:grid-cols-[2fr_1fr]` → `xl:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]`
   (คอลัมน์ `fr` มีขั้นต่ำเป็น min-content เหมือนกัน)
2. `Field` และ `TextInput` / `Select` / `Textarea` → เพิ่ม `min-w-0`
   ⚠️ **ความกว้างขั้นต่ำของ `<select>` เท่ากับตัวเลือกที่ยาวที่สุด** — หน้านี้มีตัวเลือกยาว ๆ
   อย่าง "บันทึกข้อความ · บันทึกข้อความภายในหน่วยงาน" และรายการหน่วยงาน 372 หน่วย
3. แถวปุ่มท้ายฟอร์ม → `flex-wrap` (ปุ่มมี `shrink-0` ติดมาจาก `buttonVariants` จึงไม่ยอมหด)

⚠️ **`min-w-0` บน `Field` ห้ามถอด** — มีคอมเมนต์อธิบายเหตุผลคาไว้ในไฟล์ทั้งสองที่แล้ว
เพราะเป็นคลาสที่ดู "ไม่มีเหตุผล" ที่สุดที่จะโดนคนอื่นลบทิ้งตอนเก็บกวาด

#### วิธีหาตัวการที่ได้ผลจริง — ซ่อน subtree ทีละตัวแล้ววัดว่าหน้าหดไหม

ไม่ใช่เดาจากคลาสที่เห็นใน markup · วัดออกมาได้ตรง ๆ ว่า
**รายการเลือกหน่วยงานดันอยู่ 387px และแถวปุ่มดันอีก 90px** จึงรู้ว่าต้องแก้ทั้งสองที่
ไม่ใช่แก้ที่เดียวแล้วคิดว่าจบ

#### ผล

| ชุด | ผล |
| --- | --- |
| responsive | ✅ 74/74 ครบทุกหน้าทุกความกว้าง |
| unit | ✅ 166/166 |
| e2e ทั้งชุด | ✅ 111/111 |

push แล้ว (`1f8e0d1`/`ede3780` · 27 ส.ค. 2569 เย็น) · **ผล CI ตรวจแล้วเขียวครบ ดู §23.19**

### 23.19 ✅ ผล CI ของ `ede3780` — เขียวครบสามงาน (ตรวจ 28 ส.ค. 2569)

ของค้างข้อแรกที่ค้างมาตั้งแต่ §23.18 คือ "push แล้วแต่ยังไม่มีใครดูผล" · เปิดหน้า Actions ดูแล้ว
(`gh` CLI ไม่มีบนเครื่องนี้ · https://github.com/rattanawong-it/e-saraban/actions)

**CI #6 · commit `ede3780` · Status Success · รวม 2m 51s**

| งาน | เวลา | ผล |
| --- | --- | --- |
| รูปแบบโค้ด · typecheck · unit test | 48s | ✅ 13 ไฟล์ · 166 เคส |
| integration test (Postgres จริง) | 1m 0s | ✅ 10 ไฟล์ · 124 เคส |
| e2e (Playwright บน production build) | 2m 31s | ✅ **111 เคส ใน 1.3 นาที** |

**ความกังวลเรื่อง timeout ไม่เกิดขึ้นจริง** — e2e ที่โตจาก 31 เป็น 111 เคสวิ่งจบใน 1.3 นาที
ห่างจากเพดาน `webServer.timeout` (600 วิ) มาก · **ไม่ต้องขยายอะไรทั้งนั้น**
ที่ประเมินไว้ว่า "จะยาวกว่า 137 วินาทีเดิมมาก" ผิด เพราะลืมนับว่า Playwright รันหลาย worker ขนาน
ชุด responsive 74 เคสเป็นเคสเปิดหน้าแล้ววัดความกว้าง ไม่ได้กดอะไรต่อ จึงถูกมาก

**CI #5 (`1f8e0d1`) ขึ้น cancelled — ไม่ใช่เทสต์พัง** · โดน concurrency ตัดเพราะ push `ede3780`
ตามมาห่างกันสามนาที · `ede3780` ครอบโค้ดชุดเดียวกันและเขียว จึงไม่มีอะไรต้องตาม

⚠️ **ทั้งสามงานเตือน Node.js 20 deprecated** — `actions/checkout@v4` · `actions/setup-node@v4` ·
...
### 23.21 ⚠️ กับดัก `getByRole("alert")` กับ route announcer ของ Next (28 ส.ค. 2569)

CI #10 แดงที่เคส "รหัสเหตุผลที่ไม่รู้จักต้องไม่ขึ้นข้อความอะไรเลย" ทั้งที่เคสเดียวกัน
เขียวใน #9 และ #11 · ทำซ้ำบนเครื่องได้ **7 ใน 10 ครั้ง** — ไม่ใช่ฟลุ๊กของ runner

**ตัวการ:** Next ใส่ตัวอ่านชื่อหน้าให้ screen reader มาเองทุกหน้า

```html
<next-route-announcer>  ← shadow DOM
  <div id="__next-route-announcer__" role="alert" aria-live="assertive">
```

- `document.querySelectorAll('[role="alert"]')` **มองไม่เห็น** เพราะอยู่ใน shadow DOM
- `page.getByRole("alert")` ของ Playwright **เห็น** เพราะเจาะ shadow DOM ให้
- มันถูกสร้างหลัง hydration → เทสต์กลายเป็นการแข่งเวลา ตรวจก่อน hydration ผ่าน ตรวจหลังแดง

**สองหน้าที่โดน และโดนคนละแบบ**

| ที่ | เดิม | อาการ |
| --- | --- | --- |
| `smoke.spec.ts` | `expect(getByRole("alert")).toHaveCount(0)` | แดงแบบสุ่ม |
| `document-flow.spec.ts` | `expect(getByRole("alert").or(...).first()).toBeVisible()` | **เขียวปลอม** — announcer มีขนาด 1×1px จึงนับว่า visible ผ่านด่านได้เองโดยไม่มีคำเตือนจริงบนหน้า |

**ทางแก้:** ติด `data-slot="alert"` ให้คอมโพเนนต์ `Alert` (แนวเดียวกับ `data-slot="button"`
ที่มีอยู่แล้ว) แล้วเทสต์เจาะจงด้วย `[data-slot='alert']` ซึ่งไม่ชนกับของที่เฟรมเวิร์กแถมมา
· เคสของหน้าล็อกอินเพิ่มการตรวจตรง ๆ ด้วยว่าข้อความที่ผู้โจมตียัดมาต้องไม่ถูกเรนเดอร์

**บทเรียนกว้างกว่านั้น:** ด่านที่เขียนว่า "ต้องไม่มี X" หรือ "ต้องมี X" โดยอ้าง ARIA role
มีโอกาสไปชนของที่เฟรมเวิร์กใส่มาเองเสมอ · ถ้าจะเฝ้าคอมโพเนนต์ของเรา ให้ติดป้ายของเราเอง
· และ **เทสต์ที่แดงเป็นบางครั้งอย่าเพิ่งกดรันใหม่** — รอบนี้ถ้ากดรันใหม่แล้วเขียวจะไม่มีวันรู้ว่า
มีเคสอีกตัวที่เขียวปลอมมาตลอด

ผลหลังแก้: e2e 114/114 ผ่าน · รันเฉพาะสามเคสของหน้าล็อกอินซ้ำ 5 รอบ ผ่านทั้ง 15 ครั้ง

### 23.22 ✅ แผง "ความเคลื่อนไหวล่าสุด" บนหน้าภาพรวม (28 ส.ค. 2569)

ผู้ดูแลทักว่าแผงนี้แสดงข้อมูลที่ไม่เกี่ยวกับผู้ใช้ทั่วไป (`docs/sample_v10.png`) ·
ไล่โค้ดแล้วเจอว่าเป็น**ของชั่วคราวจาก P1** ที่ไม่มีใครกลับมาเปลี่ยน

```ts
/** กิจกรรมล่าสุดที่ผู้ใช้คนนี้มีสิทธิ์เห็น — ใช้แทน "เอกสารล่าสุด" ในเฟส P1 */
```

ตอนนั้นระบบยังไม่มีเอกสารให้แสดง จึงอ่าน `auditLog` มาโชว์ไปพลางก่อน · พอ P2 สร้าง
เอกสารจริงแล้วแผงนี้ยังเป็น audit ดิบ ๆ อยู่ ผลคือหน้าแรกขึ้น "เข้าสู่ระบบด้วย Google" ·
"ออกจากระบบ" · "ผู้ดูแลรีเซ็ตรหัสผ่านให้" ปนกับงานเอกสาร และทุกแถวกดต่อไม่ได้เลย

#### เปลี่ยนคำถามที่แผงนี้ตอบ

จาก *"ระบบบันทึกอะไรไว้บ้าง"* (คำถามของผู้ตรวจ) เป็น *"หนังสือที่ฉันเกี่ยวข้องขยับไปถึงไหน"*
· อ่านจาก **`DocumentAction`** ซึ่งเป็นตารางที่ออกแบบมาเป็น timeline ของผู้ใช้อยู่แล้ว
กรองด้วย `documentVisibilityWhere()` ตัวเดียวกับหน้ารายการ

| เดิม | ใหม่ |
| --- | --- |
| ชื่อ action ของ audit | **เลขหนังสือ + ชื่อเรื่อง** กดไปที่เอกสารได้ |
| ป้าย "สำเร็จ" ทุกแถว (ไม่มีข้อมูล) | **สถานะปัจจุบันของหนังสือ** |
| `28/08/2569 14:02` | `2 วันที่แล้ว` (`formatRelativeThai`) |
| login/logout/export/งานผู้ดูแล | คัดออกทั้งหมด — มีบ้านอยู่ที่ `/admin/audit` แล้ว |
| ลิงก์ "ดูทั้งหมด" ไป `/admin/audit` (คนทั่วไปเข้าไม่ได้ ปุ่มจึงหายทั้งปุ่ม) | ไป `/search` ทุกคนใช้ได้ |

**คัด `CREATED`/`UPDATED`/แนบไฟล์ออก** — เป็นสิ่งที่เจ้าของเรื่องเพิ่งทำเองและมีปริมาณ
มากที่สุด (บนฐาน dev รอบหนึ่ง `CREATED` มี 2,739 แถวจาก 11,887) ถ้าปล่อยเข้ามา
เหตุการณ์ที่ต้องรู้จริงอย่าง "ถูกตีกลับ" จะถูกดันตกขอบภายในไม่กี่นาที

**ยุบให้เหลือฉบับละแถว** — หนังสือฉบับเดียวขยับหลายจังหวะติดกันได้ (ส่งให้สารบรรณ →
ออกเลข → เวียน) ไม่ยุบแล้วฉบับเดียวกินแผงทั้งแผง · ดึงมา `take * 4` แล้วคัดในหน่วยความจำ

⚠️ **ชื่อเรื่องเอกสารชั้น 1–3 ผ่าน `safeSubject()` ซ้ำอีกชั้น** แม้ด่านมองเห็นจะกรองคนที่
ไม่มี ACL ออกไปแล้ว เพราะแผงนี้อยู่หน้าแรกที่คนเดินผ่านหลังจอเห็นได้ง่ายที่สุดในระบบ ·
มีเทสต์ integration คุมไว้

#### ไม่มีแถบเลื่อนแล้ว (ผู้ดูแลสั่งในรอบเดียวกัน)

ถอด `overflow-y-auto` ออกจากทั้งการ์ด "รอฉันรับทราบ" และ "ความเคลื่อนไหวล่าสุด" ·
คุมความยาวด้วยจำนวนแถวที่ service คืนมา (5) ไม่ใช่ด้วยการซ่อนส่วนที่ล้น ·
การ์ดความเคลื่อนไหวเลิกยืดเต็มพื้นที่ที่เหลือ ไม่งั้นจะเหลือช่องว่างในกรอบก้อนใหญ่

⚠️ **เคส e2e ที่เคยบังคับกติกาเก่าถูกเขียนใหม่** — เดิมชื่อ "รายการบนหน้าภาพรวมเลื่อนใน
กรอบตัวเอง" และ **บังคับให้ต้องมี** `overflowY === "auto"` · ตอนนี้กลับกันคือห้ามมี
แต่ยังคงเจตนาเดิมไว้: "ไม่มีแถบเลื่อน" ทำให้ผ่านได้ด้วยการตัดเนื้อหาทิ้งเหมือนกัน
(การ์ดมี `overflow-hidden`) เคสใหม่จึงตรวจด้วยว่า**แถวสุดท้ายยังอยู่ในจอจริง**

ผลตรวจ: unit 187 · integration 134 → **136** · e2e 114 ผ่านครบ

### 23.23 ✅ ข้อมูลตัวอย่างสำหรับดูหน้าจอ — `pnpm db:seed:demo` (28 ส.ค. 2569)

ฐาน dev มีหนังสือแค่ 3 ฉบับและ **ไม่มีหนังสือส่งภายนอกเลยสักฉบับ** หน้าจอหลายหน้าจึงว่าง
จนดูไม่ออกว่าของจริงจะหน้าตาอย่างไร · เพิ่มสคริปต์สร้าง **3 ทิศทาง × 3 ระยะของวงจร = 9 ฉบับ**

| ทิศทาง | ระยะที่สร้างให้ |
| --- | --- |
| หนังสือภายใน (บันทึกข้อความ) | ร่าง · รอออกเลข · ออกเลข + เวียนแล้ว |
| หนังสือส่งภายนอก | ร่าง · รอออกเลข · ออกเลข + ส่งออกแล้ว |
| หนังสือรับ | ลงทะเบียนรับ · ส่งต่อหน่วยงาน · ปิดเรื่อง |

**ยิงผ่าน service จริงทั้งหมด ไม่ INSERT ตรง ๆ** — หน้าภาพรวม กล่องเอกสาร ทะเบียน และ
การแจ้งเตือน อ่านจากตารางคนละชุด (`DocumentAction` · `DocumentRecipient` · `NumberSequence` ·
`Notification`) การยัดแถวเข้า `documents` อย่างเดียวจะได้ข้อมูลที่ "มีอยู่" แต่หน้าจอยังว่าง
และอาจได้สถานะที่ state machine จริงสร้างไม่ได้ ทำให้ทดสอบไปแล้วไม่ตรงกับของจริง

**สองข้อที่ตั้งใจทำและอย่าเผลอเปลี่ยน**

1. **ชั้นความลับของ ctx อ่านจากแถวจริงของผู้ใช้** ไม่ตั้งเป็น 3 ตายตัว — ข้อมูลที่สร้างด้วย
   สิทธิ์ที่คนคนนั้นไม่มีจริง จะทำให้ดูหน้าจอแล้วเห็นสิ่งที่ผู้ใช้จริงไม่มีวันเห็น
2. **ไม่แตะ `clearanceLevel` ของบัญชีใน seed** — เป็นกับดักเดียวกับที่บันทึกไว้ใน
   `tests/integration/fixtures/users.ts` ที่เคยทำให้เทสต์แดงยกชุด

⚠️ **ห้ามรันบนเครื่องจริง** — ทุกฉบับที่ออกเลขกินเลขทะเบียนจริงและคืนไม่ได้ตาม §6.4 ·
`--clean` ลบเอกสารได้ (soft delete) แต่ **คืนเลขไม่ได้** เหมือน teardown ของ e2e ·
สคริปต์ปฏิเสธการรันเมื่อ `NODE_ENV=production`

**ตรวจแล้วว่าไม่ไปกวนเทสต์** — ข้อมูลนี้อยู่บนฐาน dev ที่ integration/e2e ใช้ร่วมกัน
รันครบทั้งสองชุดหลังสร้างข้อมูล: integration 136 · e2e 114 ผ่านเหมือนเดิม
(เพราะทุกชุดสร้างของตัวเองและวัดผลแบบเทียบก่อน-หลัง ไม่ได้ผูกกับจำนวนคงที่บนฐาน)

### 23.24 ✅ หน้าภาพรวม — สองการ์ดล่างวางคู่กัน แก้แถวถูกตัดขาด (28 ส.ค. 2569)

ผู้ดูแลทักสองรอบ: แถวในการ์ด "รอฉันรับทราบ" ถูกตัดขาดกลางบรรทัด (`docs/sample_v11.png`)
แล้วตามด้วยเรื่องช่องไฟระหว่างส่วนที่ชนกัน (`docs/sample_v12.png`)

#### ต้นเหตุ: หน้าล็อกความสูงไว้หนึ่งจอ แต่ของสองการ์ดวางซ้อนกัน

วัดจริงที่ 1280×720 — พื้นที่ทั้งหน้า **596px** · การ์ดสถิติกิน 146px เหลือให้สองการ์ดล่าง
**418px** แต่วางซ้อนกันต้องการ **~550px** · พอถอด `overflow-y-auto` ออกไปตามที่ผู้ดูแลสั่ง
(§23.22) สิ่งที่เหลือคือ `overflow-hidden` ซึ่ง **ตัดแถวสุดท้ายขาดกลางบรรทัดแทน**

| | ก่อน | หลัง |
| --- | --- | --- |
| การ์ด "รอฉันรับทราบ" | client 143 / scroll 218 → **ตัดทิ้ง 75px** | 218/218 |
| การ์ด "ความเคลื่อนไหว" | client 217 / scroll 332 → **ตัดทิ้ง 115px** | 332/332 |

**แก้ด้วยการวางคู่กันบนจอ lg ขึ้นไป** (`lg:grid-cols-2`) — ความสูงคิดจากการ์ดที่สูงกว่า
ไม่ใช่ผลรวม · ใส่ `lg:items-start` ด้วย ไม่งั้นกริดจะยืดการ์ดสั้นให้สูงเท่าการ์ดยาว
แล้วเหลือช่องว่างในกรอบก้อนใหญ่แบบเดิม

#### ช่องไฟ (sample_v12)

การ์ดในกริดถูกตั้ง `lg:mt-0` และตัวกริดไม่มีระยะบน → **ช่องไฟระหว่างการ์ดสถิติกับสองการ์ดล่าง
เป็นศูนย์** สองส่วนจึงชนกัน · ย้ายระยะไปไว้ที่กริดที่เดียว (`mt-4 lg:mt-5` + `lg:gap-5`)
การ์ดไม่ต้องถือ margin ของตัวเองอีก

⚠️ **จำนวนแถวผูกกับความสูงของหน้าโดยตรง** — ที่ 1280×720 ใส่ได้การ์ดละ **5 แถว**
เมื่อเว้นช่องไฟ 20px · ลองเพิ่มเป็น 6 แล้ววัดได้ว่าหน้าล้นกรอบ 15px และแถวสุดท้ายถูกตัดทันที
จึงกลับมาที่ 5 · เขียนหมายเหตุคาไว้ที่ `getRecentActivity()` แล้ว

#### เทสต์ที่ปล่อยบั๊กนี้ผ่านมาได้ — แก้แล้วสองจุด

1. **เคสเดิมดูแค่ `overflow: auto/scroll`** จึงมองไม่เห็นกรอบที่ `overflow-hidden` แล้วตัดเนื้อหาทิ้ง
   · เปลี่ยนเป็นตรวจทุกกรอบที่ `overflowY !== "visible"` แล้ว `scrollHeight > clientHeight`
2. **`e2e.runner` ไม่เคยมีหนังสือรอรับทราบเลยสักฉบับ** การ์ดนั้นจึงไม่ถูกเรนเดอร์ในเทสต์
   แม้แต่ครั้งเดียว · เพิ่ม `ensureE2EAwaitingAck()` สร้างสามฉบับ (มี/ไม่มีกำหนดวัน ·
   สามระดับความด่วน) แล้วยืนยันว่าเคสใหม่จับบั๊กได้จริงก่อนแก้

#### จัดหน้าแถวให้เป็นระบบเดียวกัน

สองการ์ดนี้อยู่ติดกันแต่เดิมวางคนละแบบ — ตอนนี้ทั้งคู่เป็น **เลขหนังสือนำหน้าชื่อเรื่อง
ในบรรทัดเดียวกัน** แล้วบรรทัดรองเป็นข้อมูลประกอบ · บรรทัดรองของ "รอฉันรับทราบ"
**ต้องมีเสมอ** แม้ไม่มีกำหนดวัน (ขึ้นว่า "ไม่ได้กำหนดวันรับทราบ") ไม่งั้นแถวสูงไม่เท่ากัน
และป้ายด้านขวาจะเยื้องกันทั้งคอลัมน์ · เวลาแบบสัมพัทธ์ย้ายจากริมขวาไปบรรทัดรอง
คืนที่ให้ชื่อเรื่องราว 70px หลังคอลัมน์แคบลงครึ่งหนึ่ง

---

## 24. 🔵 จุดที่หยุดงานไว้ — 28 ส.ค. 2569 (กลับมาทำต่อสัปดาห์หน้า)

**push ครบถึง `5de2ee9`** · working tree สะอาด (เหลือ `docs/sample_*.png` ที่ตั้งใจไม่ track)

### สิ่งแรกที่ต้องทำเมื่อกลับมา

1. **เปิด Docker Desktop ก่อน** — container `esaraban-postgres` ขึ้นเองหลัง engine พร้อม
   ลืมแล้วจะพังแบบหาสาเหตุยาก (`next build` พ่น `prisma:error` แต่ build ผ่านเงียบ ๆ)
2. ⚠️ **ยังไม่มีใครดูผล CI #14 (`30074b0`) และ #15 (`5de2ee9`)**
   https://github.com/rattanawong-it/e-saraban/actions ·
   บนเครื่องรันครบก่อน push แล้ว: **unit 187 · integration 136 · e2e 114** ผ่านหมด
3. ข้อมูลตัวอย่างอยู่บนฐาน dev แล้วจาก `pnpm db:seed:demo` — ไม่ต้องสร้างซ้ำ

### งานโค้ดที่พร้อมทำต่อ

| งาน | หมายเหตุ |
| --- | --- |
| ปุ่มถอนการผูกบัญชี Google | รหัส audit `auth.identity.unlinked` เตรียมไว้แล้ว ยังไม่มีปุ่มให้กด |
| หน้า "ตั้งรหัสผ่าน" | สำหรับบัญชีที่ `passwordHash` เป็น null (เข้าด้วย Google อย่างเดียว) |
| **retention + partition ของ audit** | ⚠️ ข้อเดียวที่ทำทีหลังแล้วแพง — ลบแถวทำ hash chain ขาด ต้องเป็น archive/partition + checkpoint ไม่ใช่ `DELETE` |
| ค่าเริ่มต้นหน้า `/admin/audit` | ให้เปิดมาเห็น DENY/CRITICAL ก่อน แทน "ทั้งหมด" 11,887 แถว (งานครึ่งชั่วโมง) |

### รอคนตอบ (เดินต่อเองไม่ได้)

**UAT** (ก้อนใหญ่สุด ยังไม่เคยทำ · `docs/uat-script.md`) · **ข้อความ banner**
(`docs/information-banners.md` ร่างครบรอตรวจ) · **§15 ข้อ 4 · 6 · 7** ·
**§15 ข้อ 8 (ข)** ยืนยันจำกัดโดเมน `krirk.ac.th` · **(ง)** บังคับ 2FA สำหรับเอกสารชั้น 2–3 หรือไม่

⚠️ **ก่อนขึ้นเครื่องจริง: reset client secret ของ Google** — กุญแจชุดที่ใช้อยู่ผ่านหน้าแชตมา
และ **§15 ข้อ 7 (TLS certificate) ยังค้าง** ซึ่งบล็อกทั้งการขึ้นระบบและ Google Sign-In

### สิ่งที่เรียนรู้รอบนี้ อย่าให้กลับมาเป็นบั๊กซ้ำ

- **§23.21** `getByRole("alert")` ชนกับ route announcer ของ Next ใน shadow DOM
- **§23.24** `overflow-hidden` ที่ไม่มีแถบเลื่อน = ตัดเนื้อหาทิ้งเงียบ ๆ · เทสต์ต้องจับทั้งสองอาการ
- **§23.20** `prisma migrate dev` เป็น interactive ใช้บนเอเจนต์ไม่ได้ → `migrate diff` + `migrate deploy`
- **§23.20** `where: { tenantId: undefined }` ของ Prisma = ไม่มีตัวกรองเลย ไม่ใช่หาค่า null
