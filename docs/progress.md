...
| ชุด | ผล |
|---|---|
| e2e | **19 เคส เขียวหมด** (setup 1 · smoke 11 · document-flow 2 · report-export 5) · 37.9 วิ |
| unit · integration | ไม่ได้แตะ — ยังเป็น 151 / 118 เท่าเดิม |

`typecheck · lint` ผ่าน · เจ็ดเคสของ `document-flow` กับ `report-export` **ผ่านตั้งแต่รอบแรก
ไม่ต้องแก้โค้ดแอปเลยสักบรรทัด**

### 23.8 ⏳ ของที่ยังค้างใน P5

1. **Notification in-app** — ยังไม่เริ่ม (`NotificationAdapter` interface มีมาตั้งแต่ §206 แล้ว)
2. **คู่มือผู้ใช้ + UAT** — ยังไม่เริ่ม
