...
| # | คำถาม | กระทบขั้นไหน |
|---|-------|-------------|
| 3 | ~~ผังหน่วยงานจริง + รหัสหนังสือ~~ | ✅ **ได้แล้ว 25 ส.ค. 2569** — รหัส 6 หลัก 371 หน่วย · spec §16 + `docs/org-units.csv` · **ยังต้องเขียนลง `prisma/seed.ts` แล้ว seed ใหม่บนฐานข้อมูลเปล่า** |
...
**สิ่งที่ต้องทำก่อนงานอื่นใน P2** — เขียนผังจริงลง `prisma/seed.ts` (แทน `ORG_TREE` 11 หน่วยของ P1)
แล้ว `pnpm db:seed` ใหม่บนฐานข้อมูลเปล่า · ต้องเพิ่มฟิลด์ `canIssueNumber` บน `OrgUnit` ด้วย (D15)
